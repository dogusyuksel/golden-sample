...
typedef struct __SMARTCARD_HandleTypeDef {
    ...
    void (*AbortTransmitCpltCallback)(
        struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Abort Transmit Complete Callback */

    void (*AbortReceiveCpltCallback)(
        struct __SMARTCARD_HandleTypeDef *hsc); /*!< SMARTCARD Abort Receive Complete Callback  */
...
typedef void (*pSMARTCARD_CallbackTypeDef)(
    SMARTCARD_HandleTypeDef *hsc); /*!< pointer to an SMARTCARD callback function */
...
#define __HAL_SMARTCARD_ENABLE_IT(__HANDLE__, __INTERRUPT__)                                                           \
    ((((__INTERRUPT__) >> 28U) == SMARTCARD_CR1_REG_INDEX)                                                             \
         ? ((__HANDLE__)->Instance->CR1 |= ((__INTERRUPT__)&SMARTCARD_IT_MASK))                                        \
...
#define __HAL_SMARTCARD_GET_IT_SOURCE(__HANDLE__, __IT__)                                                              \
    (((((__IT__) >> 28U) == SMARTCARD_CR1_REG_INDEX) ? (__HANDLE__)->Instance->CR1 : (__HANDLE__)->Instance->CR3) &    \
     (((uint32_t)(__IT__)) & SMARTCARD_IT_MASK))
...
#define IS_SMARTCARD_STOPBITS(STOPBITS)                                                                                \
    (((STOPBITS) == SMARTCARD_STOPBITS_0_5) || ((STOPBITS) == SMARTCARD_STOPBITS_1_5))
...
#define IS_SMARTCARD_LASTBIT(LASTBIT)                                                                                  \
    (((LASTBIT) == SMARTCARD_LASTBIT_DISABLE) || ((LASTBIT) == SMARTCARD_LASTBIT_ENABLE))
...
#define SMARTCARD_DIVFRAQ(__PCLK__, __BAUD__)                                                                          \
    ((((SMARTCARD_DIV((__PCLK__), (__BAUD__)) - (SMARTCARD_DIVMANT((__PCLK__), (__BAUD__)) * 100U)) * 16U) + 50U) /    \
     100U)
...
#define SMARTCARD_BRR(__PCLK__, __BAUD__)                                                                              \
    (((SMARTCARD_DIVMANT((__PCLK__), (__BAUD__)) << 4U) + (SMARTCARD_DIVFRAQ((__PCLK__), (__BAUD__)) & 0xF0U)) +       \
