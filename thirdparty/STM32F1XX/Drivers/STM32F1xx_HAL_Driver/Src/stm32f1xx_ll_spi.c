...
#define SPI_CR1_CLEAR_MASK                                                                                             \
    (SPI_CR1_CPHA | SPI_CR1_CPOL | SPI_CR1_MSTR | SPI_CR1_BR | SPI_CR1_LSBFIRST | SPI_CR1_SSI | SPI_CR1_SSM |          \
     SPI_CR1_RXONLY | SPI_CR1_DFF | SPI_CR1_CRCNEXT | SPI_CR1_CRCEN | SPI_CR1_BIDIOE | SPI_CR1_BIDIMODE)
/**
 * @}
 */
 ...
 * @note   As some bits in SPI configuration registers can only be written when the SPI is disabled (SPI_CR1_SPE bit
    ...
    if (LL_SPI_IsEnabled(SPIx) == 0x00000000U) {
        ...
        MODIFY_REG(SPIx->CR1, SPI_CR1_CLEAR_MASK,
                   SPI_InitStruct->TransferDirection | SPI_InitStruct->Mode | SPI_InitStruct->DataWidth |
                       ...
                       SPI_InitStruct->BaudRate | SPI_InitStruct->BitOrder | SPI_InitStruct->CRCCalculation);
 ...
 *          - ERROR: SPI registers are not de-initialized
 */
ErrorStatus LL_I2S_DeInit(SPI_TypeDef *SPIx) { return LL_SPI_DeInit(SPIx); }
