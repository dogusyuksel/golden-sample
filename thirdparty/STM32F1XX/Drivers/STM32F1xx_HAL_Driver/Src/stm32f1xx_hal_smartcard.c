...
static HAL_StatusTypeDef SMARTCARD_WaitOnFlagUntilTimeout(SMARTCARD_HandleTypeDef *hsc, uint32_t Flag,
    ...
    if (HAL_SMARTCARD_STATE_READY == hsc->gState) {
        ...
        case HAL_SMARTCARD_ABORT_TRANSMIT_COMPLETE_CB_ID:
            hsc->AbortTransmitCpltCallback =
                HAL_SMARTCARD_AbortTransmitCpltCallback; /* Legacy weak AbortTransmitCpltCallback */
            break;

        case HAL_SMARTCARD_ABORT_RECEIVE_COMPLETE_CB_ID:
            hsc->AbortReceiveCpltCallback =
                HAL_SMARTCARD_AbortReceiveCpltCallback; /* Legacy weak AbortReceiveCpltCallback  */
            break;
...
@verbatim
 ...
 [..]
    ...
    (#) For Abort services based on interrupts (HAL_SMARTCARD_Abortxxx_IT), a set of Abort Complete Callbacks are
provided:
...
. Received character is then retrieved and stored in Rx buffer, Error code is set to allow user to identify error type,
       ...
       (+) Error is considered as Blocking : Transfer could not be completed properly and is aborted.
           This concerns Frame Error in Interrupt mode transmission, Overrun Error in Interrupt mode reception and all
errors in DMA mode. Error code is set to allow user to identify error type, and HAL_SMARTCARD_ErrorCallback() user
 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SMARTCARD_Transmit(SMARTCARD_HandleTypeDef *hsc, const uint8_t *pData, uint16_t Size,
                                         uint32_t Timeout) {
...
HAL_StatusTypeDef HAL_SMARTCARD_Abort_IT(SMARTCARD_HandleTypeDef *hsc) {
    ...
    /* If DMA Tx and/or DMA Rx Handles are associated to SMARTCARD Handle, DMA Abort complete callbacks should be
...
void HAL_SMARTCARD_IRQHandler(SMARTCARD_HandleTypeDef *hsc) {
    ...
    if ((errorflags != RESET) &&
        ...
        if (((isrflags & SMARTCARD_FLAG_ORE) != RESET) &&
 ...
 */
uint32_t HAL_SMARTCARD_GetError(const SMARTCARD_HandleTypeDef *hsc) { return hsc->ErrorCode; }
...
static void SMARTCARD_SetConfig(SMARTCARD_HandleTypeDef *hsc) {
    ...
    tmpreg |= (uint32_t)(USART_CR2_CLKEN | hsc->Init.CLKPolarity | hsc->Init.CLKPhase | hsc->Init.CLKLastBit |
                         hsc->Init.StopBits);
    /* Write to USART CR2 */
