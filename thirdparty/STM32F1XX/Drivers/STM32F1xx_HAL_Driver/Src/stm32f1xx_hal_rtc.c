...
HAL_StatusTypeDef HAL_RTC_SetTime(RTC_HandleTypeDef *hrtc, RTC_TimeTypeDef *sTime, uint32_t Format) {
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        counter_time = (uint32_t)(((uint32_t)sTime->Hours * 3600U) + ((uint32_t)sTime->Minutes * 60U) +
        ...
        counter_time =
            (((uint32_t)(RTC_Bcd2ToByte(sTime->Hours)) * 3600U) + ((uint32_t)(RTC_Bcd2ToByte(sTime->Minutes)) * 60U) +
...
HAL_StatusTypeDef HAL_RTC_SetDate(RTC_HandleTypeDef *hrtc, RTC_DateTypeDef *sDate, uint32_t Format) {
    ...
    hrtc->DateToUpdate.WeekDay =
        RTC_WeekDayNum(hrtc->DateToUpdate.Year, hrtc->DateToUpdate.Month, hrtc->DateToUpdate.Date);
...
HAL_StatusTypeDef HAL_RTC_SetAlarm(RTC_HandleTypeDef *hrtc, RTC_AlarmTypeDef *sAlarm, uint32_t Format) {
    ...
    counter_time =
        (uint32_t)(((uint32_t)stime.Hours * 3600U) + ((uint32_t)stime.Minutes * 60U) + ((uint32_t)stime.Seconds));
    ...
    if (Format == RTC_FORMAT_BIN) {
        ...
        counter_alarm = (uint32_t)(((uint32_t)sAlarm->AlarmTime.Hours * 3600U) +
                                   ((uint32_t)sAlarm->AlarmTime.Minutes * 60U) + ((uint32_t)sAlarm->AlarmTime.Seconds));
    } else {
...
static void RTC_DateUpdate(RTC_HandleTypeDef *hrtc, uint32_t DayElapsed) {
    ...
    for (loop = 0U; loop < DayElapsed; loop++) {
        if ((month == 1U) || (month == 3U) || (month == 5U) || (month == 7U) || (month == 8U) || (month == 10U) ||
            (month == 12U)) {
            if (day < 31U) {
                day++;
            }
...
static uint8_t RTC_WeekDayNum(uint32_t nYear, uint8_t nMonth, uint8_t nDay) {
    ...
    if (nMonth < 3U) {
        ...
        weekday = (((23U * nMonth) / 9U) + nDay + 4U + year + ((year - 1U) / 4U) - ((year - 1U) / 100U) +
                   ((year - 1U) / 400U)) %
                  7U;
    } else {
