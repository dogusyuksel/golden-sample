...
HAL_StatusTypeDef HAL_SD_WriteBlocks(SD_HandleTypeDef *hsd, uint8_t *pData, uint32_t BlockAdd, uint32_t NumberOfBlocks,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        dataremaining = config.DataLength;
        while (!__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT |
...
HAL_StatusTypeDef HAL_SD_ReadBlocks_IT(SD_HandleTypeDef *hsd, uint8_t *pData, uint32_t BlockAdd,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        __HAL_SD_ENABLE_IT(
            hsd, (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_RXOVERR | SDIO_IT_DATAEND | SDIO_FLAG_RXFIFOHF));
...
HAL_StatusTypeDef HAL_SD_WriteBlocks_IT(SD_HandleTypeDef *hsd, uint8_t *pData, uint32_t BlockAdd,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        __HAL_SD_ENABLE_IT(
            hsd, (SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_DATAEND | SDIO_FLAG_TXFIFOHE));
...
HAL_StatusTypeDef HAL_SD_ReadBlocks_DMA(SD_HandleTypeDef *hsd, uint8_t *pData, uint32_t BlockAdd,
    ...
    if (hsd->State == HAL_SD_STATE_READY) {
        ...
        if (HAL_DMA_Start_IT(hsd->hdmarx, (uint32_t)&hsd->Instance->FIFO, (uint32_t)pData,
                             (uint32_t)(BLOCKSIZE * NumberOfBlocks) / 4U) != HAL_OK) {
...
void HAL_SD_IRQHandler(SD_HandleTypeDef *hsd) {
    ...
    if ((__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_RXFIFOHF) != RESET) && ((context & SD_CONTEXT_IT) != 0U)) {
        ...
        __HAL_SD_DISABLE_IT(hsd, SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR |
    ...
    else if (__HAL_SD_GET_FLAG(hsd, SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_RXOVERR | SDIO_FLAG_TXUNDERR) !=
             RESET) {
        /* Set Error code */
        ...
        if ((context & SD_CONTEXT_IT) != 0U) {
            ...
            } else if (((context & SD_CONTEXT_READ_SINGLE_BLOCK) != 0U) ||
                       ((context & SD_CONTEXT_READ_MULTIPLE_BLOCK) != 0U)) {
...
HAL_StatusTypeDef HAL_SD_Abort(SD_HandleTypeDef *hsd) {
    ...
    __HAL_SD_DISABLE_IT(hsd,
                        SDIO_IT_DATAEND | SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_TXUNDERR | SDIO_IT_RXOVERR);
