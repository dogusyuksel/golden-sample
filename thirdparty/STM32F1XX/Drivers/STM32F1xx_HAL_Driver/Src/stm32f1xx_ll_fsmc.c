...
#define BTR_CLEAR_MASK                                                                                                 \
    ((uint32_t)(FSMC_BTRx_ADDSET | FSMC_BTRx_ADDHLD | FSMC_BTRx_DATAST | FSMC_BTRx_BUSTURN | FSMC_BTRx_CLKDIV |        \
                FSMC_BTRx_DATLAT | FSMC_BTRx_ACCMOD))

/* --- BWTR Register ---*/
/* BWTR register clear mask */
#if defined(FSMC_BWTRx_BUSTURN)
#define BWTR_CLEAR_MASK                                                                                                \
    ((uint32_t)(FSMC_BWTRx_ADDSET | FSMC_BWTRx_ADDHLD | FSMC_BWTRx_DATAST | FSMC_BWTRx_BUSTURN | FSMC_BWTRx_ACCMOD))
#else
#define BWTR_CLEAR_MASK                                                                                                \
    ((uint32_t)(FSMC_BWTRx_ADDSET | FSMC_BWTRx_ADDHLD | FSMC_BWTRx_DATAST | FSMC_BWTRx_ACCMOD | FSMC_BWTRx_CLKDIV |    \
                FSMC_BWTRx_DATLAT))
...
#define PCR_CLEAR_MASK                                                                                                 \
    ((uint32_t)(FSMC_PCRx_PWAITEN | FSMC_PCRx_PBKEN | FSMC_PCRx_PTYP | FSMC_PCRx_PWID | FSMC_PCRx_ECCEN |              \
                FSMC_PCRx_TCLR | FSMC_PCRx_TAR | FSMC_PCRx_ECCPS))
/* --- PMEM Register ---*/
/* PMEM register clear mask */
#define PMEM_CLEAR_MASK                                                                                                \
    ((uint32_t)(FSMC_PMEMx_MEMSETx | FSMC_PMEMx_MEMWAITx | FSMC_PMEMx_MEMHOLDx | FSMC_PMEMx_MEMHIZx))

/* --- PATT Register ---*/
/* PATT register clear mask */
#define PATT_CLEAR_MASK                                                                                                \
    ((uint32_t)(FSMC_PATTx_ATTSETx | FSMC_PATTx_ATTWAITx | FSMC_PATTx_ATTHOLDx | FSMC_PATTx_ATTHIZx))
...
/* --- PMEM Register ---*/
/* PMEM register clear mask */
#define PMEM4_CLEAR_MASK                                                                                               \
    ((uint32_t)(FSMC_PMEM4_MEMSET4 | FSMC_PMEM4_MEMWAIT4 | FSMC_PMEM4_MEMHOLD4 | FSMC_PMEM4_MEMHIZ4))

/* --- PATT Register ---*/
/* PATT register clear mask */
#define PATT4_CLEAR_MASK                                                                                               \
    ((uint32_t)(FSMC_PATT4_ATTSET4 | FSMC_PATT4_ATTWAIT4 | FSMC_PATT4_ATTHOLD4 | FSMC_PATT4_ATTHIZ4))
...
HAL_StatusTypeDef FSMC_NORSRAM_Init(FSMC_NORSRAM_TypeDef *Device, FSMC_NORSRAM_InitTypeDef *Init) {
    ...
    btcr_reg = (flashaccess | Init->DataAddressMux | Init->MemoryType | Init->MemoryDataWidth | Init->BurstAccessMode |
                Init->WaitSignalPolarity | Init->WaitSignalActive | Init->WriteOperation | Init->WaitSignal |
                Init->ExtendedMode | Init->AsynchronousWait | Init->WriteBurst);

    btcr_reg |= Init->WrapMode;
    btcr_reg |= Init->PageSize;

    mask = (FSMC_BCRx_MBKEN | FSMC_BCRx_MUXEN | FSMC_BCRx_MTYP | FSMC_BCRx_MWID | FSMC_BCRx_FACCEN | FSMC_BCRx_BURSTEN |
            FSMC_BCRx_WAITPOL | FSMC_BCRx_WAITCFG | FSMC_BCRx_WREN | FSMC_BCRx_WAITEN | FSMC_BCRx_EXTMOD |
            FSMC_BCRx_ASYNCWAIT | FSMC_BCRx_CBURSTRW);
...
HAL_StatusTypeDef FSMC_NORSRAM_Timing_Init(FSMC_NORSRAM_TypeDef *Device, FSMC_NORSRAM_TimingTypeDef *Timing,
    ...
    MODIFY_REG(Device->BTCR[Bank + 1U], BTR_CLEAR_MASK,
               (Timing->AddressSetupTime | ((Timing->AddressHoldTime) << FSMC_BTRx_ADDHLD_Pos) |
                ((Timing->DataSetupTime) << FSMC_BTRx_DATAST_Pos) |
                ((Timing->BusTurnAroundDuration) << FSMC_BTRx_BUSTURN_Pos) |
                (((Timing->CLKDivision) - 1U) << FSMC_BTRx_CLKDIV_Pos) |
                (((Timing->DataLatency) - 2U) << FSMC_BTRx_DATLAT_Pos) | (Timing->AccessMode)));
...
HAL_StatusTypeDef FSMC_NORSRAM_Extended_Timing_Init(FSMC_NORSRAM_EXTENDED_TypeDef *Device,
    ...
    if (ExtendedMode == FSMC_EXTENDED_MODE_ENABLE) {
        ...
        MODIFY_REG(Device->BWTR[Bank], BWTR_CLEAR_MASK,
                   (Timing->AddressSetupTime | ((Timing->AddressHoldTime) << FSMC_BWTRx_ADDHLD_Pos) |
                    ((Timing->DataSetupTime) << FSMC_BWTRx_DATAST_Pos) | Timing->AccessMode |
...
HAL_StatusTypeDef FSMC_NAND_Init(FSMC_NAND_TypeDef *Device, FSMC_NAND_InitTypeDef *Init) {
    ...
    if (Init->NandBank == FSMC_NAND_BANK2) {
        ...
        MODIFY_REG(Device->PCR2, PCR_CLEAR_MASK,
                   (Init->Waitfeature | FSMC_PCR_MEMORY_TYPE_NAND | Init->MemoryDataWidth | Init->EccComputation |
                    Init->ECCPageSize | ((Init->TCLRSetupTime) << FSMC_PCRx_TCLR_Pos) |
...
HAL_StatusTypeDef FSMC_NAND_CommonSpace_Timing_Init(FSMC_NAND_TypeDef *Device, FSMC_NAND_PCC_TimingTypeDef *Timing,
    ...
    if (Bank == FSMC_NAND_BANK2) {
        /* NAND bank 2 registers configuration */
        MODIFY_REG(Device->PMEM2, PMEM_CLEAR_MASK,
                   (Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PMEMx_MEMWAITx_Pos) |
                    ((Timing->HoldSetupTime) << FSMC_PMEMx_MEMHOLDx_Pos) |
                    ((Timing->HiZSetupTime) << FSMC_PMEMx_MEMHIZx_Pos)));
    } else {
        /* NAND bank 3 registers configuration */
        MODIFY_REG(Device->PMEM3, PMEM_CLEAR_MASK,
                   (Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PMEMx_MEMWAITx_Pos) |
                    ((Timing->HoldSetupTime) << FSMC_PMEMx_MEMHOLDx_Pos) |
                    ((Timing->HiZSetupTime) << FSMC_PMEMx_MEMHIZx_Pos)));
    }
...
HAL_StatusTypeDef FSMC_NAND_AttributeSpace_Timing_Init(FSMC_NAND_TypeDef *Device, FSMC_NAND_PCC_TimingTypeDef *Timing,
    ...
    if (Bank == FSMC_NAND_BANK2) {
        /* NAND bank 2 registers configuration */
        MODIFY_REG(Device->PATT2, PATT_CLEAR_MASK,
                   (Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PATTx_ATTWAITx_Pos) |
                    ((Timing->HoldSetupTime) << FSMC_PATTx_ATTHOLDx_Pos) |
                    ((Timing->HiZSetupTime) << FSMC_PATTx_ATTHIZx_Pos)));
    } else {
        /* NAND bank 3 registers configuration */
        MODIFY_REG(Device->PATT3, PATT_CLEAR_MASK,
                   (Timing->SetupTime | ((Timing->WaitSetupTime) << FSMC_PATTx_ATTWAITx_Pos) |
                    ((Timing->HoldSetupTime) << FSMC_PATTx_ATTHOLDx_Pos) |
                    ((Timing->HiZSetupTime) << FSMC_PATTx_ATTHIZx_Pos)));
    }
...
HAL_StatusTypeDef FSMC_PCCARD_Init(FSMC_PCCARD_TypeDef *Device, FSMC_PCCARD_InitTypeDef *Init) {
    ...
    MODIFY_REG(Device->PCR4, (FSMC_PCRx_PTYP | FSMC_PCRx_PWAITEN | FSMC_PCRx_PWID | FSMC_PCRx_TCLR | FSMC_PCRx_TAR),
               (FSMC_PCR_MEMORY_TYPE_PCCARD | Init->Waitfeature | FSMC_NAND_PCC_MEM_BUS_WIDTH_16 |
                (Init->TCLRSetupTime << FSMC_PCRx_TCLR_Pos) | (Init->TARSetupTime << FSMC_PCRx_TAR_Pos)));
