...
void arm_radix2_butterfly_inverse_f32(float32_t *pSrc, uint32_t fftLen, float32_t *pCoef, uint16_t twidCoefModifier,
...
void arm_cfft_radix2_f32(const arm_cfft_radix2_instance_f32 *S, float32_t *pSrc) {
    ...
    if (S->ifftFlag == 1U) {
        ...
        arm_radix2_butterfly_inverse_f32(pSrc, S->fftLen, S->pTwiddle, S->twidCoefModifier, S->onebyfftLen);
    } else {
        /*  Complex FFT radix-2  */
