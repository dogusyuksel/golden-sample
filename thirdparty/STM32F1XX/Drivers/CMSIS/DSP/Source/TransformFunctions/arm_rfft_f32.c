...
extern void arm_radix4_butterfly_inverse_f32(float32_t *pSrc, uint16_t fftLen, float32_t *pCoef,
...
void arm_split_rfft_f32(float32_t *pSrc, uint32_t fftLen, float32_t *pATable, float32_t *pBTable, float32_t *pDst,
                        uint32_t modifier);

void arm_split_rifft_f32(float32_t *pSrc, uint32_t fftLen, float32_t *pATable, float32_t *pBTable, float32_t *pDst,
                         uint32_t modifier);
...
void arm_rfft_f32(const arm_rfft_instance_f32 *S, float32_t *pSrc, float32_t *pDst) {
    ...
    if (S->ifftFlagR == 1U) {
        ...
        arm_radix4_butterfly_inverse_f32(pDst, S_CFFT->fftLen, S_CFFT->pTwiddle, S_CFFT->twidCoefModifier,
                                         S_CFFT->onebyfftLen);
 ...
 * @param[out]  *pDst 				points to the output buffer.
 * @param[in]   modifier 	        twiddle coefficient modifier that supports different size FFTs with the same
