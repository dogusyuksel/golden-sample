 ...
 *initialized. Set the values in the state buffer to zeros and then manually initialize the instance structure as
 *follows: <pre> arm_fir_lattice_instance_f32 S = {numStages, pState, pCoeffs}; arm_fir_lattice_instance_q31 S =
 *{numStages, pState, pCoeffs}; arm_fir_lattice_instance_q15 S = {numStages, pState, pCoeffs};
 * </pre>
 * \par
 * where <code>numStages</code> is the number of stages in the filter; <code>pState</code> is the address of the state
 *buffer; <code>pCoeffs</code> is the address of the coefficient buffer. \par Fixed-Point Behavior Care must be taken
 *when using the fixed-point versions of the FIR Lattice filter functions. In particular, the overflow and saturation
 *behavior of the accumulator used in each function must be considered. Refer to the function specific documentation
 *below for usage guidelines.
 */
