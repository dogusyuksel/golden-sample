 ...
 *
 * \par Algorithm:
 * The sparse filter instant structure contains an array of tap indices <code>pTapDelay</code> which specifies the
 *locations of the non-zero coefficients. This is in addition to the coefficient array <code>b</code>. The
 *implementation essentially skips the multiplications by zero and leads to an efficient realization. <pre> y[n] = b[0]
 ** x[n-pTapDelay[0]] + b[1] * x[n-pTapDelay[1]] + b[2] * x[n-pTapDelay[2]] + ...+ b[numTaps-1] *
 ...
 *initializes each of the 4 different data type filter instance structures <pre> arm_fir_sparse_instance_f32 S =
 *{numTaps, 0, pState, pCoeffs, maxDelay, pTapDelay}; arm_fir_sparse_instance_q31 S = {numTaps, 0, pState, pCoeffs,
 *maxDelay, pTapDelay}; arm_fir_sparse_instance_q15 S = {numTaps, 0, pState, pCoeffs, maxDelay, pTapDelay};
...
void arm_fir_sparse_f32(arm_fir_sparse_instance_f32 *S, float32_t *pSrc, float32_t *pDst, float32_t *pScratchIn,
    ...
    arm_circularRead_f32((int32_t *)py, delaySize, &readIndex, 1, (int32_t *)pb, (int32_t *)pb, blockSize, 1,
                         blockSize);
