 ...
 * After all multiply-accumulates are performed, the 2.30 accumulator is truncated to 2.15 format and then saturated
 * to 1.15 format. In order to avoid overflows the input signal or coefficients must be scaled down by log2(numTaps)
 * bits.
 */
...
void arm_fir_sparse_q15(arm_fir_sparse_instance_q15 *S, q15_t *pSrc, q15_t *pDst, q15_t *pScratchIn, q31_t *pScratchOut,
    ...
    while (blkCnt > 0U) {
        ...
        *__SIMD32(pOut)++ = __PKHBT((q15_t)__SSAT(in1 >> 15, 16), (q15_t)__SSAT(in2 >> 15, 16), 16);
