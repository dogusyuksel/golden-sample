 ...
 * order to avoid overflows completely the input signal must be scaled down by <code>1/(numTaps/L)</code>. since
 * <code>numTaps/L</code> additions occur per output sample. After all multiply-accumulates are performed, the 2.62
