 ...
 * scaled down. Scale down the inputs by log2(min(srcALen, srcBLen)) (log2 is read as log to the base 2) times to avoid
 * overflows, as maximum of min(srcALen, srcBLen) number of additions are carried internally.
 *
 * \par
 * See <code>arm_conv_q31()</code> for a slower implementation of this function which uses 64-bit accumulation to
