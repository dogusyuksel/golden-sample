 ...
 * LMS filters use a gradient descent method in which the filter coefficients are updated based on the instantaneous
 *error signal. Adaptive filters are often used in communication systems, equalizers, and noise removal. The CMSIS DSP
 *Library contains LMS filter functions that operate on Q15, Q31, and floating-point data types. The library also
 *contains normalized LMS filters in which the filter coefficient adaptation is indepedent of the level of the input
 *signal.
 *
 ...
 *input. This "error signal" tends towards zero as the filter adapts. The LMS processing functions accept the input and
 ...
 *
 * \par
 * After each sample of the error signal is computed, the filter coefficients <code>b[k]</code> are updated on a
 *sample-by-sample basis: <pre> b[k] = b[k] + e[n] * mu * x[n-k],  for k=0, 1, ..., numTaps-1
 * </pre>
 ...
 *initializes each of the 3 different data type filter instance structures <pre> arm_lms_instance_f32 S = {numTaps,
 *pState, pCoeffs, mu}; arm_lms_instance_q31 S = {numTaps, pState, pCoeffs, mu, postShift}; arm_lms_instance_q15 S =
 *{numTaps, pState, pCoeffs, mu, postShift};
 * </pre>
 * where <code>numTaps</code> is the number of filter coefficients in the filter; <code>pState</code> is the address of
 *the state buffer; <code>pCoeffs</code> is the address of the coefficient buffer; <code>mu</code> is the step size
 *parameter; and <code>postShift</code> is the shift applied to coefficients.
 *
