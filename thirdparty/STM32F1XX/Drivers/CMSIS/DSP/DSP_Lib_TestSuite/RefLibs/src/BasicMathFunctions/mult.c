...
void ref_mult_q15(q15_t *pSrcA, q15_t *pSrcB, q15_t *pDst, uint32_t blockSize) {
    ...
    for (i = 0; i < blockSize; i++) {
        temp = ((q31_t)pSrcA[i] * pSrcB[i]) >>
               15; // this comment is for JD, this is specifically 15 and not 16 like the q31 case might imply.  This is
                   // because CMSIS DSP lib does it this way.  No other reason.
