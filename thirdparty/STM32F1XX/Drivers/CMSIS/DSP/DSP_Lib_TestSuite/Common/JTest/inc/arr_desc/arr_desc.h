...
#define ARR_DESC_INTERNAL_DEFINE(name, data_ptr, element_count, element_size)                                          \
    ARR_DESC_t name = {data_ptr, element_count, element_size, element_count * element_size}
...
#define ARR_DESC_DEFINE_SUBSET(name, original, element_cnt)                                                            \
    ARR_DESC_INTERNAL_DEFINE(name, &ARR_DESC_ARR_NAME(original), element_cnt,                                          \
 ...
 */
#define ARR_DESC_DECLARE(name) extern ARR_DESC_t name /* Note the lacking semicolon */

/**
 *  Evaluate to the number of bytes stored in the #ARR_DESC_t.
 */
#define ARR_DESC_BYTES(arr_desc_ptr) ((arr_desc_ptr)->element_count * (arr_desc_ptr)->element_size)
...
#define ARR_DESC_MEMSET(arr_desc_ptr, value, bytes)                                                                    \
    do {                                                                                                               \
        memset((arr_desc_ptr)->data_ptr, value, BOUND(0, (arr_desc_ptr)->underlying_size, bytes));                     \
    } while (0)
...
#define ARR_DESC_COPY(arr_desc_dest_ptr, arr_desc_src_ptr)                                                             \
    do {                                                                                                               \
        if (ARR_DESC_COPYABLE(arr_desc_dest_ptr, arr_desc_src_ptr)) {                                                  \
            ARR_DESC_MEMCPY(arr_desc_dest_ptr, arr_desc_src_ptr, ARR_DESC_BYTES(arr_desc_src_ptr));                    \
...
#define ARR_DESC_MEMCMP(arr_desc_ptr_a, arr_desc_ptr_b, bytes)                                                         \
    memcmp((arr_desc_ptr_a)->data_ptr, (arr_desc_ptr_b)->data_ptr, bytes) /* Note the lacking semicolon */

/**
 *  Zero out the contents of the #ARR_DESC_t.
 */
#define ARR_DESC_ZERO(arr_desc_ptr) ARR_DESC_MEMSET(arr_desc_ptr, 0, (arr_desc_ptr)->underlying_size)

/**
 *  Evaluate to the data address in #ARR_DESC_t at offset.
 */
#define ARR_DESC_DATA_ADDR(type, arr_desc_ptr, offset) ((void *)(((type *)((arr_desc_ptr)->data_ptr)) + offset))

/**
 *  Evaluate to the element in #ARR_DESC_t with type at idx.
 */
#define ARR_DESC_ELT(type, idx, arr_desc_ptr) (*((type *)ARR_DESC_DATA_ADDR(type, arr_desc_ptr, idx)))
