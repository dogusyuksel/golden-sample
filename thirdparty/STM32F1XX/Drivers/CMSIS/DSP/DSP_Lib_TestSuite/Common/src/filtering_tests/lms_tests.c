...
#define LMS_DEFINE_TEST(suffix, config_suffix, output_type, mu)                                                        \
    JTEST_DEFINE_TEST(arm_lms##config_suffix##_##suffix##_test, arm_lms##config_suffix##_##suffix) {                   \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            TEMPLATE_DO_ARR_DESC(                                                                                      \
                ...
                arm_fir_init_##suffix(&fir_inst, numTaps, (output_type *)filtering_coeffs_##suffix,                    \
                                      (void *)filtering_pState, blockSize);                                            \
                                                                                                                       \
                ref_fir_##suffix(&fir_inst, (void *)filtering_##suffix##_inputs, (void *)filtering_input_lms,          \
                                 blockSize);                                                                           \
                                                                                                                       \
                for (i = 0; i < numTaps; i++) { *((output_type *)filtering_coeffs_lms + i) = (output_type)0; }         \
                                                                                                                       \
                for (i = 0; i < blockSize; i++) {                                                                      \
                    /* scaled down so that lms will converge           */                                              \
                    /* scaled down by almost the max of the abs(input) */                                              \
                    *((output_type *)filtering_input_lms + i) = *((output_type *)filtering_input_lms + i) / 200.0f;    \
                                                                                                                       \
                ...
                arm_lms##config_suffix##_init_##suffix(&lms_inst_fut, numTaps, (output_type *)filtering_coeffs_lms,    \
                                                       (void *)filtering_pState, mu, blockSize);                       \
                                                                                                                       \
                JTEST_COUNT_CYCLES(arm_lms##config_suffix##_##suffix(                                                  \
                    &lms_inst_fut, (void *)filtering_output_f32_fut, (void *)filtering_input_lms,                      \
                    (void *)filtering_output_fut, (void *)((output_type *)filtering_output_fut + blockSize),           \
                    blockSize));                                                                                       \
                                                                                                                       \
                for (i = 0; i < numTaps; i++) { *((output_type *)filtering_coeffs_lms + i) = (output_type)0; }         \
                                                                                                                       \
                arm_lms##config_suffix##_init_##suffix(&lms_inst_ref, numTaps, (output_type *)filtering_coeffs_lms,    \
                                                       (void *)filtering_pState, mu, blockSize);                       \
                                                                                                                       \
                ref_lms##config_suffix##_##suffix(&lms_inst_ref, (void *)filtering_output_f32_fut,                     \
    ...
    JTEST_DEFINE_TEST(arm_lms##config_suffix##_##suffix##_test, arm_lms##config_suffix##_##suffix) {                   \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            TEMPLATE_DO_ARR_DESC(                                                                                      \
                ...
                TEMPLATE_DO_ARR_DESC(                                                                                  \
                    postshifts_idx, uint8_t, postShift, filtering_postshifts, /* Initialize the FIR Instances */       \
                    arm_fir_init_##suffix(&fir_inst, numTaps, (output_type *)filtering_coeffs_##suffix,                \
                                          (void *)filtering_pState, blockSize);                                        \
                                                                                                                       \
                    ref_fir_##suffix(&fir_inst, (void *)filtering_##suffix##_inputs, (void *)filtering_input_lms,      \
                    ...
                    arm_lms##config_suffix##_init_##suffix(                                                            \
                        &lms_inst_fut, numTaps, (output_type *)filtering_coeffs_lms, (void *)filtering_pState,         \
                        mu_##suffix, blockSize, postShift);                                                            \
                                                                                                                       \
                    JTEST_COUNT_CYCLES(arm_lms##config_suffix##_##suffix(                                              \
                        &lms_inst_fut, (void *)filtering_output_f32_fut, (void *)filtering_input_lms,                  \
                        (void *)filtering_output_fut, (void *)((output_type *)filtering_output_fut + blockSize),       \
                    ...
                    for (i = 0; i < numTaps; i++) { *((output_type *)filtering_coeffs_lms + i) = (output_type)0; }     \
                                                                                                                       \
                    arm_lms##config_suffix##_init_##suffix(                                                            \
                        &lms_inst_ref, numTaps, (output_type *)filtering_coeffs_lms, (void *)filtering_pState,         \
                        mu_##suffix, blockSize, postShift);                                                            \
                                                                                                                       \
                    ref_lms##config_suffix##_##suffix(&lms_inst_ref, (void *)filtering_output_f32_fut,                 \
                                                      (void *)filtering_input_lms, (void *)filtering_output_ref,       \
