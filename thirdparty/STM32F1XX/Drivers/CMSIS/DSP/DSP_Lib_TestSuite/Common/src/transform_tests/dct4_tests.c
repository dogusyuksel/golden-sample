...
#define DCT4_DEFINE_TEST(suffix, input_type)                                                                           \
    JTEST_DEFINE_TEST(arm_dct4_##suffix##_test, arm_dct4_##suffix) {                                                   \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            arm_dct4_init_##suffix(&dct4_inst_fut, &rfft_inst_fut, &cfft_inst_fut, fftlen, fftlen / 2, normalize);     \
                                                                                                                       \
            arm_dct4_init_##suffix(&dct4_inst_ref, &rfft_inst_ref, &cfft_inst_ref, fftlen, fftlen / 2, normalize);     \
                                                                                                                       \
            memset(transform_fft_input_fut, 0, fftlen * sizeof(input_type));                                           \
                                                                                                                       \
            TRANSFORM_PREPARE_INPLACE_INPUTS(transform_fft_##suffix##_inputs, fftlen * sizeof(input_type));            \
                                                                                                                       \
            ...
            JTEST_COUNT_CYCLES(arm_dct4_##suffix(&dct4_inst_fut, (void *)transform_fft_input_fut,                      \
                                                 (void *)transform_fft_inplace_input_fut));                            \
                                                                                                                       \
            memset(transform_fft_input_ref, 0, fftlen * sizeof(input_type));                                           \
                                                                                                                       \
            /* Input provided as a scratch buffer. Inplace input is */ /* actual input. */                             \
...
#define DCT4_FIXED_POINT_DEFINE_TEST(suffix, input_type, prefix)                                                       \
    JTEST_DEFINE_TEST(arm_dct4_##suffix##_test, arm_dct4_##suffix) {                                                   \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            float32_t normalize_f32 = ref_sqrt_f32((2.0f / (float32_t)fftlen)); input_type normalize;                  \
                                                                                                                       \
            /* Calculate normalized DCT4 value for input_type. */                                                      \
            TEST_CONVERT_FLOAT_TO(&normalize_f32, &normalize, 1, input_type);                                          \
                                                                                                                       \
             ...
             */                                                                                                        \
            for (i = 0; i < fftlen; i++) {                                                                             \
                ((input_type *)transform_fft_inplace_input_fut)[i] = prefix##transform_fft_##suffix##_inputs[i] >> 1;  \
                ((input_type *)transform_fft_inplace_input_ref)[i] = prefix##transform_fft_##suffix##_inputs[i] >> 1;  \
            }                                                                                                          \
                                                                                                                       \
            memset(transform_fft_input_fut, 0, fftlen * sizeof(input_type));                                           \
                                                                                                                       \
