...
#define CORRELATE_arm_correlate_opt_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                        \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_fut, (void *)filtering_scratch)

#define CORRELATE_arm_correlate_opt_q7_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                     \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_fut, (void *)filtering_scratch,         \
          (void *)filtering_scratch2)

#define CORRELATE_ref_correlate_opt_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                        \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_ref, (void *)filtering_scratch)

#define CORRELATE_ref_correlate_opt_q7_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                     \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_ref, (void *)filtering_scratch,         \
          (void *)filtering_scratch2)

#define CORRELATE_arm_correlate_fast_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                       \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_fut)

#define CORRELATE_ref_correlate_fast_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                       \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_ref)

#define CORRELATE_arm_correlate_fast_opt_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                   \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_fut, (void *)filtering_scratch)

#define CORRELATE_ref_correlate_fast_opt_INPUT_INTERFACE(input_a, input_a_len, input_b, input_b_len)                   \
    PAREN(input_a, input_a_len, input_b, input_b_len, (void *)filtering_output_ref, (void *)filtering_scratch)
...
#define CORRELATE_DEFINE_RAND_INPUT_ARR_DESCS(type_suffix)                                                             \
    ARR_DESC_DEFINE_USING_ARR(TYPE_FROM_ABBREV(type_suffix), correlate_input_rand1_##type_suffix,                      \
                              CORRELATE_##type_suffix##_INPUTS, 0, CORRELATE_MAX_INPUT_ELTS);                          \
                                                                                                                       \
    ARR_DESC_DEFINE_USING_ARR(TYPE_FROM_ABBREV(type_suffix), correlate_input_rand2_##type_suffix,                      \
...
#define CORRELATE_DEFINE_ALL_INPUTS(type_suffix)                                                                       \
    ARR_DESC_DEFINE(ARR_DESC_t *, correlate_##type_suffix##_a_inputs, 4,                                               \
                    CURLY(&correlate_input_zeros, &correlate_input_zeros, &correlate_input_rand1_##type_suffix,        \
                          &correlate_input_rand1_##type_suffix));                                                      \
    ARR_DESC_DEFINE(ARR_DESC_t *, correlate_##type_suffix##_b_inputs, 4,                                               \
                    CURLY(&correlate_input_zeros, &correlate_input_rand1_##type_suffix, &correlate_input_zeros,        \
                          &correlate_input_rand2_##type_suffix)) /* Note the lacking semicolon */
 ...
 */
ARR_DESC_DEFINE(uint32_t, correlate_lens_a, 5, CURLY(1, 1, 17, 15, CORRELATE_MAX_INPUT_ELTS));

ARR_DESC_DEFINE(uint32_t, correlate_lens_b, 5, CURLY(1, 2, 1, CORRELATE_MAX_INPUT_ELTS, CORRELATE_MAX_INPUT_ELTS));

/*--------------------------------------------------------------------------------*/
/* Convolution Tests */
/*--------------------------------------------------------------------------------*/

#define CORRELATE_TEST_TEMPLATE(fut, fut_arg_interface, ref, ref_arg_interface, suffix, output_type)                   \
    JTEST_DEFINE_TEST(fut##_tests, fut) {                                                                              \
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            void *input_b_ptr = ARR_DESC_ELT(ARR_DESC_t *, input_idx, &(correlate_##suffix##_b_inputs))->data_ptr;     \
                                                                                                                       \
            TEMPLATE_DO_ARR_DESC(                                                                                      \
                correlate_len_idx, uint32_t, correlate_len_a, correlate_lens_a,                                        \
                uint32_t correlate_len_b = ARR_DESC_ELT(uint32_t, correlate_len_idx, &(correlate_lens_b));             \
                                                                                                                       \
                ...
                JTEST_DUMP_STRF("Input A Length: %d\n"                                                                 \
                                ...
                                (int)correlate_len_a, (int)correlate_len_b);                                           \
                                                                                                                       \
                memset(filtering_output_ref, 0, (2 * CORRELATE_MAX_INPUT_ELTS) * sizeof(output_type));                 \
                memset(filtering_output_fut, 0, (2 * CORRELATE_MAX_INPUT_ELTS) * sizeof(output_type));                 \
                                                                                                                       \
                TEST_CALL_FUT_AND_REF(                                                                                 \
                    fut, fut_arg_interface(input_a_ptr, correlate_len_a, input_b_ptr, correlate_len_b), ref,           \
                    ref_arg_interface(input_a_ptr, correlate_len_a, input_b_ptr, correlate_len_b));                    \
                                                                                                                       \
                FILTERING_SNR_COMPARE_INTERFACE(correlate_len_a + correlate_len_b - 2, output_type)));                 \
                                                                                                                       \
        return JTEST_TEST_PASSED;                                                                                      \
    }

#define CORRELATE_DEFINE_TEST(fn_name, suffix, output_type, test_template)                                             \
    test_template(arm_##fn_name##_##suffix, CORRELATE_arm_##fn_name##_INPUT_INTERFACE, ref_##fn_name##_##suffix,       \
                  CORRELATE_ref_##fn_name##_INPUT_INTERFACE, suffix, output_type) /* Note the lacking semicolon*/
...
CORRELATE_TEST_TEMPLATE(arm_correlate_opt_q7, CORRELATE_arm_correlate_opt_q7_INPUT_INTERFACE, ref_correlate_opt_q7,
                        CORRELATE_ref_correlate_opt_q7_INPUT_INTERFACE, q7, q7_t);
