...
*/
#define RFFT_DEFINE_TEST(suffix, config_suffix, ifft_flag, input_type, output_type)                                    \
    JTEST_DEFINE_TEST(arm_rfft_##suffix##_##config_suffix##_test, arm_rfft_##suffix) {                                 \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            arm_rfft_init_##suffix(&rfft_inst_fut, (uint32_t)fftlen, ifft_flag, 1U);                                   \
                                                                                                                       \
            arm_rfft_init_##suffix(&rfft_inst_ref, (uint32_t)fftlen, ifft_flag, 1U);                                   \
                                                                                                                       \
            if (ifft_flag) {                                                                                           \
                TRANSFORM_PREPARE_INVERSE_INPUTS(transform_fft_##suffix##_inputs, fftlen, input_type,                  \
                                                 fftlen * sizeof(input_type));                                         \
            } else { TRANSFORM_COPY_INPUTS(transform_fft_##suffix##_inputs, fftlen * sizeof(input_type)); }            \
                                                                                                                       \
            ...
            JTEST_COUNT_CYCLES(                                                                                        \
                arm_rfft_##suffix(&rfft_inst_fut, (void *)transform_fft_input_fut, (void *)transform_fft_output_fut)); \
                                                                                                                       \
            ref_rfft_##suffix(&rfft_inst_ref, (void *)transform_fft_input_ref, (void *)transform_fft_output_ref);      \
                                                                                                                       \
            /* Test correctness */                                                                                     \
            TRANSFORM_SNR_COMPARE_INTERFACE(fftlen, output_type));                                                     \
                                                                                                                       \
