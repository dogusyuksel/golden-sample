...
#define CFFT_FN_NAME(fn_specifier, type_suffix) arm_cfft_##fn_specifier##_##type_suffix
 ...
 *  for an inverse FFT.
 */
#define CFFT_FAMILY_DEFINE_TEST(fn_specifier, type_suffix, config_suffix, /* Delineate between test configs*/          \
    ...
    JTEST_DEFINE_TEST(CFFT_TEST_NAME(fn_specifier, type_suffix, config_suffix),                                        \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            ...
            arm_cfft_##fn_specifier##_init_##type_suffix(&cfft_inst_fut, fftlen, ifft_flag, (uint8_t)1);               \
            arm_cfft_##fn_specifier##_init_##type_suffix(&cfft_inst_ref, fftlen, ifft_flag, (uint8_t)1);               \
                                                                                                                       \
            TRANSFORM_PREPARE_INPLACE_INPUTS(transform_fft_##type_suffix##_inputs,                                     \
                                             fftlen * sizeof(TYPE_FROM_ABBREV(type_suffix)) * 2 /*complex_inputs*/);   \
                                                                                                                       \
            ...
            JTEST_COUNT_CYCLES(                                                                                        \
                arm_cfft_##fn_specifier##_##type_suffix(&cfft_inst_fut, (void *)transform_fft_inplace_input_fut));     \
                                                                                                                       \
            ref_cfft_##fn_specifier##_##type_suffix(&cfft_inst_ref, (void *)transform_fft_inplace_input_ref);          \
                                                                                                                       \
            /* Test correctness */                                                                                     \
...
#define CFFT_FAMILY_DEFINE_ALL_TESTS(config_suffix, ifft_flag)                                                         \
    /* Radix2 tests*/                                                                                                  \
    CFFT_FAMILY_DEFINE_TEST(radix2, q31, config_suffix, TRANSFORM_SNR_COMPARE_CMPLX_INTERFACE, TYPE_FROM_ABBREV(q31),  \
                            ifft_flag);                                                                                \
    CFFT_FAMILY_DEFINE_TEST(radix2, q15, config_suffix, TRANSFORM_SNR_COMPARE_CMPLX_INTERFACE, TYPE_FROM_ABBREV(q15),  \
                            ifft_flag);                                                                                \
    /* Radix4 tests*/                                                                                                  \
    CFFT_FAMILY_DEFINE_TEST(radix4, q31, config_suffix, TRANSFORM_SNR_COMPARE_CMPLX_INTERFACE, TYPE_FROM_ABBREV(q31),  \
                            ifft_flag);                                                                                \
    CFFT_FAMILY_DEFINE_TEST(radix4, q15, config_suffix, TRANSFORM_SNR_COMPARE_CMPLX_INTERFACE, TYPE_FROM_ABBREV(q15),  \
                            ifft_flag)
/* /\* Mag tests*\/                                                  \ */
