...
#define JTEST_ARM_MAT_SCALE_TEST(suffix, type)                                                                         \
    JTEST_DEFINE_TEST(arm_mat_scale_##suffix##_test, arm_mat_scale_##suffix) {                                         \
        ...
        TEMPLATE_DO_ARR_DESC(                                                                                          \
            mat_idx, arm_matrix_instance_##suffix *, mat_ptr, matrix_##suffix##_b_inputs,                              \
            MATRIX_TEST_CONFIG_SAMESIZE_OUTPUT(arm_matrix_instance_##suffix *, mat_ptr);                               \
                                                                                                                       \
            for (i = 0; i < MATRIX_MAX_COEFFS_LEN; i++) {                                                              \
                for (j = 0; j < MATRIX_MAX_SHIFTS_LEN; j++) {                                                          \
                    JTEST_DUMP_STRF("Matrix Dimensions: %dx%d\n", (int)mat_ptr->numRows, (int)mat_ptr->numCols);       \
                                                                                                                       \
                    JTEST_COUNT_CYCLES(arm_mat_scale_##suffix(mat_ptr, matrix_##suffix##_scale_values[i],              \
                                                              matrix_shift_values[j],                                  \
                                                              (arm_matrix_instance_##suffix *)&matrix_output_fut));    \
                                                                                                                       \
                    ref_mat_scale_##suffix(mat_ptr, matrix_##suffix##_scale_values[i], matrix_shift_values[j],         \
