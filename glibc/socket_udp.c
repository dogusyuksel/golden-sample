...
static struct option parameters[] = {{"client", no_argument, 0, 'c'},
...
void work_as_client(char *portnum) {
  ...
  clientsideaddr.sin6_port =
      htons(9192); // 0 makes OS selected suitable port number
  clientsideaddr.sin6_family = AF_INET6;
  clientsideaddr.sin6_addr = in6addr_loopback;

  s = bind(sock, (struct sockaddr *)&clientsideaddr,
  ...
  for (rp = result; rp != NULL; rp = rp->ai_next) {
    ...
    case AF_INET6:
      if (!inet_ntop(AF_INET6,
                     &(((struct sockaddr_in6 *)rp->ai_addr)->sin6_addr), ip,
                     sizeof(ip))) {
        freeaddrinfo(result); /* No longer needed */
        exit(1);
      }
  ...
  if (sendto(sock, "selam", 6, 0, (struct sockaddr *)rp->ai_addr,
             sizeof(struct sockaddr_in6)) != 6)
    fprintf(stderr, "Error sending response\n");

  if (recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr *)&peer_addr,
...
void work_as_server(char *portnum) {
  ...
  for (;;) {
    ...
    if (sendto(sfd, "selam2", 7, 0, (struct sockaddr *)&peer_addr,
               peer_addr_len) != 7)
      fprintf(stderr, "Error sending response\n");
  }
}
