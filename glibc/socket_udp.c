...
static struct option parameters[] = {{"client", no_argument, 0, 'c'}, {"help", no_argument, 0, 'h'}, {NULL, 0, 0, 0}};
...
void work_as_client(char *portnum) {
    ...
    hints.ai_flags = AI_NUMERICSERV; // this is for numeric port //AI_PASSIVE;
                                     // /* For wildcard IP address */
    hints.ai_protocol = IPPROTO_UDP;

    s = getaddrinfo(NULL, portnum, &hints,
                    &result); // since null, return addres will be loopback, "::1"
                              // could be written aswell
    if (s != 0) {
    ...
    if (sendto(sock, "selam", 6, 0, (struct sockaddr *)rp->ai_addr, sizeof(struct sockaddr_in6)) != 6)
        fprintf(stderr, "Error sending response\n");

    if (recvfrom(sock, buffer, sizeof(buffer), 0, (struct sockaddr *)&peer_addr, (socklen_t *)&peer_addr_len) == -1)
        perror("recvfrom");

    char host[NI_MAXHOST], service[NI_MAXSERV];

    s = getnameinfo((struct sockaddr *)&peer_addr, peer_addr_len, host, NI_MAXHOST, service, NI_MAXSERV,
                    NI_NUMERICSERV);
...
void work_as_server(char *portnum) {
    ...
    for (;;) {
        ...
        if (sendto(sfd, "selam2", 7, 0, (struct sockaddr *)&peer_addr, peer_addr_len) != 7)
            fprintf(stderr, "Error sending response\n");
    }
}
