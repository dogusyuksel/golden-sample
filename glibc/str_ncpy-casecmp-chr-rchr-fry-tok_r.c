...
int main(void) {
  ...
  printf("s_strncpy lenght is %lu while strlen is %lu, \"%s\"-\"%s\"\n",
         strlen(s_strncpy), strlen(s_strncpy_source), s_strncpy,
         s_strncpy_source);

  // RETURN: s_strncpy lenght is 10 while strlen is 22, "this is st"-"this is
  // strncpy string"
  ...
  printf("strchr is: \"%s\", strrchr is: \"%s\"\n", strchr(s_strchr, (int)'s'),
         strrchr(s_strchr, (int)'s'));
