...
* Internal Node: Internal Node means that a node which has at least a single
child.
* External Node: External Node means that a node which has no children. It is
also known as leaf.
...
* Depth: You can calculate depth by the number of edges from node to the root of
the tree.
...
In a proper binary tree, every internal node has exactly wo children. A binary
tree that is not proper is improper

* Balanced Binary Tree
A balanced binary tree, also referred to as a height-balanced binary tree, is
defined as a binary tree in which the height of the left and right (node to
leaf, depth is node to root) subtree of any node differ by not more than 1. To
do that, all nodes' lef and right subtrees also should be balanced

* Full Binary Tree
every parent node / internal node has either two or no children (it means, if
all leafes have siblings)18

* Perfect Binary Tree
all internal nodes have two children and all leafes are at same level

* Complete Binary Tree
every level must be completely filled. all the leaf elements must lean towards
the left. the last leaf element might not have a right sibling.
