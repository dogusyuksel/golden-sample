...
int main(void) {
    // it should be used instead of realloc because newsize prm of it lead us to
    // set it wrong
    ...
    printf("s_strndup lenght is %lu while strlen is %lu, \"%s\"-\"%s\"\n", strlen(s_strndup), strlen(s_strndup_source),
           s_strndup, s_strndup_source);

    free(s_strndup);

    // RETURN: s_strndup lenght is 10 while strlen is 22, "this is st"-"this is
    // strndup string"
    ...
    printf("s_strndupa lenght is %lu while strlen is %lu, \"%s\"-\"%s\"\n", strlen(s_strndupa),
           strlen(s_strndupa_source), s_strndupa, s_strndupa_source);

    // no need to free it

    // RETURN: s_strndupa lenght is 10 while strlen is 23, "this is st"-"this is
    // strndupa string"
