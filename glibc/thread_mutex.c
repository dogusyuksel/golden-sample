...
lock. Sometimes, a thread needs to simultaneously access two or more different
...
already locked. Both threads will remain blocked indefinitely. The simplest way
to avoid such deadlocks is to define a mutex hierarchy. When threads can lock
the same set of mutexes, they should always lock them in the same order. An
alternative strategy that is less frequently used is “try, and then back off.”
...
int main(void) {
  ...
  if (pthread_create(&(thread_id1), NULL, &trythis, (void *)&passed_data1) !=
      0) {
    perror("error while creating the thread\n");
    exit(EXIT_FAILURE);
  }
  if (pthread_create(&(thread_id2), NULL, &trythis, (void *)&passed_data2) !=
      0) {
    perror("error while creating the thread\n");
    exit(EXIT_FAILURE);
  }
  ...
  // note: if "PTHREAD_MUTEX_INITIALIZER" is used while initializing, then
  // destroy is needed
