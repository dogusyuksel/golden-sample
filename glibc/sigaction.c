...
void posix_print_stack_trace() {
  ...
  for (i = 0; i < trace_size; ++i) {
    ...
    if (strstr(messages[i], icky_global_program_name)) {
      if (!(getaddr(messages[i], addr, 32) &&
            !addr2line(icky_global_program_name, addr))) {
...
void posix_signal_handler(int sig, siginfo_t *siginfo, void *context) {
  ...
  case SIGBUS:
    ...
    case BUS_OBJERR:
      ...
      break;
    case BUS_MCEERR_AR:
      fputs("Caught SIGBUS: (Hardware memory error consumed on a machine "
            "check; action required)\n",
            stderr);
      break;
    case BUS_MCEERR_AO:
      fputs("Caught SIGBUS: (Hardware memory error detected in process but not "
