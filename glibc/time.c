...
static void display_clock(clockid_t clock, const char *name) {
  struct timespec ts;
  ...
  if (clock == CLOCK_REALTIME) {
    ...
    printf("	 %d %d %d - %d:%d:%d.%ld\n", brokentime->tm_mday,
           brokentime->tm_mon, brokentime->tm_year, brokentime->tm_hour,
           brokentime->tm_min, brokentime->tm_sec, (ts.tv_nsec / 1000000));
  }
}
...
int main(void) {
  ...
  while (1) {
    ...
    // Counts from system boot but it does not count time that the system is
    // suspended
